...
trait Source[+Out] extends FlowOps[Out] {
   ...
   */
  def append[T](flow: Flow[Out, T]): Source[T]

  /**
   * Connect this source to a sink, concatenating the processing steps of both.
   */
  def append(sink: Sink[Out]): RunnableFlow
...
/**
 * TODO:ban change the comments
 */
object Source {
  /**
   * Helper to create `Flow` with [[Faucet]] from `Publisher`.
   *
   ...
   */
  def apply[T](publisher: Publisher[T]): Source[T] = Pipe[T].withFaucet(PublisherFaucet(publisher))

  /**
   * Helper to create `Flow` with [[Faucet]] from `Iterator`.
   ...
   * steps.
   */
  def apply[T](iterator: Iterator[T]): Source[T] = Pipe[T].withFaucet(IteratorFaucet(iterator))

  /**
   * Helper to create `Flow` with [[Faucet]] from `Iterable`.
   ...
   */
  def apply[T](iterable: immutable.Iterable[T]): Source[T] = Pipe[T].withFaucet(IterableFaucet(iterable))
   ...
   */
  def apply[T](f: () ⇒ Option[T]): Source[T] = Pipe[T].withFaucet(ThunkFaucet(f))
   ...
   */
  def apply[T](future: Future[T]): Source[T] = Pipe[T].withFaucet(FutureFaucet(future))
