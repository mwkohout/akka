...
trait Flow[-In, +Out] extends FlowOps[Out] {
  ...
  /**
   * Transform this flow by prepending the given processing stages.
   */
  def prepend[T](flow: Flow[T, In]): Flow[T, Out]

  /**
   * Connect a source to this flow, concatenating the processing steps of both.
   */
  def prepend(source: Source[In]): Source[Out]

  /**
   * Transform this flow by appending the given processing stages.
   */
  def append[T](flow: Flow[Out, T]): Flow[In, T]

  /**
   * Connect this flow to a sink, concatenating the processing steps of both.
   */
  def append(sink: Sink[Out]): Sink[In]
}
 ...
 * TODO:ban comment
 */
trait MaterializedFlow extends MaterializedFaucet with MaterializedDrain
