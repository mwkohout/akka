...
/**
 * This trait is a marker for a pluggable stream faucet. Concrete instances should
 * implement [[FaucetWithKey]] or [[SimpleFaucet]], otherwise a custom [[FlowMaterializer]]
 * will have to be used to be able to attach them.
 *
 * All Faucets defined in this package rely upon an ActorBasedFlowMaterializer being
 ...
 * Faucet nodes themselves (or construct an ActorBasedFlowMaterializer).
 */
trait Faucet[+In]

/**
 * A faucet that does not need to create a user-accessible object during materialization.
 */
trait SimpleFaucet[+In] extends Faucet[In] with SourceFaucet[In] {
  /**
   * Attach this faucet to the given [[org.reactivestreams.Subscriber]]. Using the given
   * [[FlowMaterializer]] is completely optional, especially if this faucet belongs to
  ...
  /**
   * This method is only used for Faucets that return true from [[#isActive]], which then must
   * implement it.
   */
  ...
  /**
   * This method indicates whether this Faucet can create a Publisher instead of being
...
/**
 * A faucet that will create an object during materialization that the user will need
 * to retrieve in order to access aspects of this faucet (could be a Subscriber, a
 * Future/Promise, etc.).
 */
trait FaucetWithKey[+In, T] extends Faucet[In] with SourceFaucet[In] {
  /**
   * Attach this faucet to the given [[org.reactivestreams.Subscriber]]. Using the given
   * [[FlowMaterializer]] is completely optional, especially if this faucet belongs to
   * a different Reactive Streams implementation. It is the responsibility of the
 ...
 */
final case class SubscriberFaucet[In]() extends FaucetWithKey[In, Subscriber[In]] {
  override def attach(flowSubscriber: Subscriber[In], materializer: ActorBasedFlowMaterializer, flowName: String): Subscriber[In] =
    flowSubscriber

  def subscriber(m: MaterializedFaucet): Subscriber[In] = m.getFaucetFor(this)
}
 ...
 */
final case class PublisherFaucet[In](p: Publisher[In]) extends SimpleFaucet[In] {
 ...
 * steps.
 */
final case class IteratorFaucet[In](iterator: Iterator[In]) extends SimpleFaucet[In] {
  override def attach(flowSubscriber: Subscriber[In], materializer: ActorBasedFlowMaterializer, flowName: String): Unit =
 ...
 */
final case class IterableFaucet[In](iterable: immutable.Iterable[In]) extends SimpleFaucet[In] {
  override def attach(flowSubscriber: Subscriber[In], materializer: ActorBasedFlowMaterializer, flowName: String): Unit =
    create(materializer, flowName).subscribe(flowSubscriber)
  override def isActive: Boolean = true
  override def create(materializer: ActorBasedFlowMaterializer, flowName: String): Publisher[In] =
 ...
 */
final case class ThunkFaucet[In](f: () ⇒ Option[In]) extends SimpleFaucet[In] {
  override def attach(flowSubscriber: Subscriber[In], materializer: ActorBasedFlowMaterializer, flowName: String): Unit =
    create(materializer, flowName).subscribe(flowSubscriber)
  override def isActive: Boolean = true
  override def create(materializer: ActorBasedFlowMaterializer, flowName: String): Publisher[In] =
 ...
 */
final case class FutureFaucet[In](future: Future[In]) extends SimpleFaucet[In] {
  override def attach(flowSubscriber: Subscriber[In], materializer: ActorBasedFlowMaterializer, flowName: String): Unit =
    create(materializer, flowName).subscribe(flowSubscriber)
  override def isActive: Boolean = true
  override def create(materializer: ActorBasedFlowMaterializer, flowName: String): Publisher[In] =
 ...
 */
final case class TickFaucet[In](initialDelay: FiniteDuration, interval: FiniteDuration, tick: () ⇒ In) extends SimpleFaucet[In] {
  override def attach(flowSubscriber: Subscriber[In], materializer: ActorBasedFlowMaterializer, flowName: String): Unit =
    create(materializer, flowName).subscribe(flowSubscriber)
  override def isActive: Boolean = true
  override def create(materializer: ActorBasedFlowMaterializer, flowName: String): Publisher[In] =
...
trait MaterializedFaucet {
  def getFaucetFor[T](faucetKey: FaucetWithKey[_, T]): T
}

trait SourceFaucet[+Out] extends Source[Out] { self: Faucet[Out] ⇒
  override type Repr[+O] = SourcePipe[O]

  private lazy val sourcePipe = Pipe[Out].withFaucet(this)

  override def append[T](flow: Flow[Out, T]): Source[T] = sourcePipe.append(flow)

  override def append(sink: Sink[Out]): RunnableFlow = sourcePipe.append(sink)
