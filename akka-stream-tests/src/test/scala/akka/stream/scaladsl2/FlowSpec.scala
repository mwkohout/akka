...
class FlowSpec extends AkkaSpec(ConfigFactory.parseString("akka.actor.debug.receive=off\nakka.loglevel=INFO")) {
  ...
  def materializeIntoSubscriberAndPublisher[In, Out](flow: Flow[In, Out]): (Subscriber[In], Publisher[Out]) = {
    val faucet = SubscriberFaucet[In]
    val drain = PublisherDrain[Out]
    val mf = flow.prepend(faucet).append(drain).run()
    (faucet.subscriber(mf), drain.publisher(mf))
  }
  ...
  "A Flow" must {
    ...
    "materialize into Publisher/Subscriber" in {
      ...
      val faucet: Publisher[String] = Source(List("1", "2", "3")).toPublisher()
      faucet.subscribe(flowIn)
    ...
    "materialize into Publisher/Subscriber and transformation processor" in {
      ...
      val faucet: Publisher[Int] = Source(List(1, 2, 3)).toPublisher()
      faucet.subscribe(flowIn)
    ...
    "subscribe Subscriber" in {
      ...
      val c1 = StreamTestKit.SubscriberProbe[String]()
      val sink: Sink[String] = flow.append(SubscriberDrain(c1))
      val faucet: Publisher[String] = Source(List("1", "2", "3")).toPublisher()
      sink.prepend(PublisherFaucet(faucet)).run()
    ...
    "perform transformation operation" in {
      ...
      val faucet = Source(List(1, 2, 3)).toPublisher()
      flow.prepend(PublisherFaucet(faucet)).consume()
    ...
    "perform transformation operation and subscribe Subscriber" in {
      ...
      val c1 = StreamTestKit.SubscriberProbe[String]()
      val sink: Sink[Int] = flow.append(SubscriberDrain(c1))
      val faucet: Publisher[Int] = Source(List(1, 2, 3)).toPublisher()
      sink.prepend(PublisherFaucet(faucet)).run()
