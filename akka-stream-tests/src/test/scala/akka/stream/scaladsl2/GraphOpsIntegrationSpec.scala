  ...
  "FlowGraphs" must {
    ...
    "support broadcast - merge layouts" in {
      ...
      val g = FlowGraph { implicit b ⇒
        ...
        bcast ~> merge
        bcast ~> Pipe[Int].map(_ + 3) ~> merge
        merge ~> Pipe[Int].grouped(10) ~> resultFuture
      }.run()
    ...
    "support wikipedia Topological_sorting 2" in {
      ...
      val g = FlowGraph { implicit b ⇒
        ...
        val in3 = IterableFaucet(List(3))
        val in5 = IterableFaucet(List(5))
        val in7 = IterableFaucet(List(7))
        ...
        // Second layer
        m11 ~> b11
        b11 ~> Pipe[Int].grouped(1000) ~> resultFuture2 // Vertex 2 is omitted since it has only one in and out
        b11 ~> m9
        b11 ~> m10
        ...
        // Third layer
        m9 ~> Pipe[Int].grouped(1000) ~> resultFuture9
        m10 ~> Pipe[Int].grouped(1000) ~> resultFuture10
