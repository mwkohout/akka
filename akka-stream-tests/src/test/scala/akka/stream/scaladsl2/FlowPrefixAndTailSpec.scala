...
class FlowPrefixAndTailSpec extends AkkaSpec {
  ...
  "PrefixAndTail" must {
    ...
    "work on empty input" in {
      ...
      val mf = Source(Nil).prefixAndTail(10).append(futureDrain).run()
    ...
    "work on short input" in {
      val futureDrain = newFutureDrain
      val mf = Source(List(1, 2, 3)).prefixAndTail(10).append(futureDrain).run()
      val fut = futureDrain.future(mf)
      val (prefix, tailFlow) = Await.result(fut, 3.seconds)
    ...
    "work on longer inputs" in {
      val futureDrain = newFutureDrain
      val mf = Source((1 to 10).iterator).prefixAndTail(5).append(futureDrain).run()
      val fut = futureDrain.future(mf)
      val (takes, tail) = Await.result(fut, 3.seconds)
      takes should be(1 to 5)

      val futureDrain2 = FutureDrain[immutable.Seq[Int]]
      val mf2 = tail.grouped(6).append(futureDrain2).run()
    ...
    "handle zero take count" in {
      val futureDrain = newFutureDrain
      val mf = Source((1 to 10).iterator).prefixAndTail(0).append(futureDrain).run()
      val fut = futureDrain.future(mf)
      val (takes, tail) = Await.result(fut, 3.seconds)
      takes should be(Nil)

      val futureDrain2 = FutureDrain[immutable.Seq[Int]]
      val mf2 = tail.grouped(11).append(futureDrain2).run()
      val fut2 = futureDrain2.future(mf2)
    ...
    "handle negative take count" in {
      val futureDrain = newFutureDrain
      val mf = Source((1 to 10).iterator).prefixAndTail(-1).append(futureDrain).run()
      val fut = futureDrain.future(mf)
      val (takes, tail) = Await.result(fut, 3.seconds)
      takes should be(Nil)
    ...
    "work if size of take is equal to stream size" in {
      val futureDrain = newFutureDrain
      val mf = Source((1 to 10).iterator).prefixAndTail(10).append(futureDrain).run()
      val fut = futureDrain.future(mf)
      val (takes, tail) = Await.result(fut, 3.seconds)
