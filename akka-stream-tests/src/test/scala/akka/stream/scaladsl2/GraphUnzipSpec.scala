  ...
  "A unzip" must {
    ...
    "unzip to two subscribers" in {
      ...
      FlowGraph { implicit b ⇒
        ...
        unzip.right ~> Pipe[String].buffer(16, OverflowStrategy.backpressure) ~> SubscriberDrain(c2)
        unzip.left ~> Pipe[Int].buffer(16, OverflowStrategy.backpressure).map(_ * 2) ~> SubscriberDrain(c1)
      }.run()
